...
class RecordMergeId(models.Model):
    ...
    def consolidate(self):
        ...
        for merge in self.filtered(lambda m: m.state == 'progress' and not m.consolidate_done):
            ...
            vals = {}
            consolidated_order = merge.consolidate_order_ids.filtered(lambda r: r.to_merge).sorted(
    ...
    def view_relation_object(self):
        ...
        if self.ttype == 'one2many':
            ...
            if not self.criteria_ids:
                ...
                action = self.env['ir.actions.act_window'].for_xml_id('record_merge',
                                                                      'record_merge_criteria_act_window')
                action.update({
