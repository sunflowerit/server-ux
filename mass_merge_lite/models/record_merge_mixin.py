...
class RecordMergeMixin(models.AbstractModel):
    ...
    _description = "Record Merge Mixin"

    name = fields.Char(required=True, string="Name")
    model_id = fields.Many2one(
        comodel_name="ir.model", required=True, string="Model", ondelete="cascade"
    )
    ...
    def fill_consolidate_field(self):
        ...
        for merge in self.filtered(lambda m: m.state == "progress"):
            ...
            fields = merge.model_id.field_id.filtered(
                lambda f: f.ttype != "one2many"
                and not f.related
                and not f.compute
                and not f.readonly
                and f.name not in MAGIC_COLUMNS
            )
            for f in fields:
                ...
                model_id = ir_model_obj.search(
                    [
                        (
                            ...
                            "=ilike",
                            fk_table,
                        ),  # _ is the any single character pattern, so finds dots
                    ]
                )
                field_ids = []
                if model_id:
                    ...
                    if (
                        not field_id
                    ):  # could be a m2m defined in model_obj (e.g. brand_ids defined in rp)
                        field_id = ir_field_obj.search(
                            [
                                ("relation_table", "=", fk_table),
                                ("column1", "=", fk_column),
    ...
    def additional_ref_field(self):
        ...
        """
        # we do NOT want to merge ir.property with res_id,
        # bc it has been merged in consolidattion
        return [
    ...
    def fill_nonrel_field(self):
        ...
        for merge in self.filtered(lambda m: m.state == "progress"):
            ...
            for nonrel_model_field_id in nonrel_model_field_ids:
                ...
                nonrel_id_field_id = nonrel_model_field_id.model_id.field_id.filtered(
                    lambda f: f.name == "res_id"
                )
...
class RecordMergeMixinIdLine(models.AbstractModel):
    ...
    _description = "Record Merge Mixin ID Line"
...
class RecordMergeMixinFieldLine(models.AbstractModel):
    ...
    _description = "Record Merge Mixin field Line"

    merge_id = fields.Many2one("record.merge.mixin")
    field_id = fields.Many2one("ir.model.fields")
    model_id = fields.Many2one("ir.model", related="field_id.model_id")
    to_merge = fields.Boolean(string="Allow merging", default=True)
...
class RecordMergeFkField(models.Model):
    ...
    field_id = fields.Many2one(domain="[('ttype', '=', 'many2one')]")
...
class RecordMergeNonrelField(models.Model):
    ...
    model_field_id = fields.Many2one("ir.model.fields")
